        ...
        <form style={{ display: 'flex', flexDirection: 'column', gap: 14 }} onSubmit={handleSubmit}>
          ...
          {status === 'error' && (
            <div style={{ ...B, fontSize: 13, color: '#ff6b6b', padding: '12px 16px', background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.3)' }}>
              Er ging iets mis. Probeer het opnieuw of mail naar guidovperformance@gmail.com
            </div>
          )}
  ...
  return (
    <div style={{ background: 'var(--dark)', color: 'var(--text)', ...B }}>
      ...
      <section style={{ minHeight: '92vh', position: 'relative', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        <Image src="/hero.jpg" alt="Guido Vols instrueert militairen op het veld" fill style={{ objectFit: 'cover', objectPosition: 'center center' }} priority />
        ...
        <div className="hero-content" style={{ position: 'relative', zIndex: 1, padding: '80px 60px', maxWidth: '600px' }}>
          ...
          <h1 style={{ ...D, fontSize: 'clamp(56px, 7vw, 96px)', lineHeight: 0.95, letterSpacing: 1, marginBottom: 10, fontWeight: 700 }}>
            JOUW<br />DOEL,<br /><span style={{ color: 'var(--orange)' }}>ONS PLAN</span>
          </h1>
          <div style={{ ...D, fontSize: 'clamp(20px, 2.5vw, 34px)', letterSpacing: 4, color: 'var(--muted)', marginBottom: 32, fontWeight: 400 }}>GV PERFORMANCE</div>
      ...
      {/* RIBBON */}
      <div className="site-ribbon" style={{ background: 'var(--dark2)', borderTop: '1px solid rgba(255,77,0,0.1)', borderBottom: '1px solid rgba(255,77,0,0.1)', padding: '18px 60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        {['Korps Mariniers', '🥇 Gouden Medaille NL Mannen Team', 'Instructeur Fysieke Training & Sport', 'NBB Boxing Trainer KSS 3', 'Looptrainer Atletiek Unie', '13+ Certificaten'].map((item) => (
          <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 6, height: 6, background: 'var(--orange)', flexShrink: 0 }} />
            <span style={{ ...B, fontSize: 11, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase' }}>{item}</span>
          </div>
        ))}
      </div>

      {/* OVER GUIDO */}
      <section id="over" className="section-about" style={{ padding: '100px 60px', background: 'var(--dark2)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
        <div className="about-photo" style={{ position: 'relative', aspectRatio: '3/4', overflow: 'hidden' }}>
          <Image src="/about.jpg" alt="Guido Vols training" fill style={{ objectFit: 'cover', objectPosition: 'center' }} sizes="50vw" />
        </div>
        <div>
          <div style={{ ...B, fontSize: 10, letterSpacing: 4, color: 'var(--orange)', textTransform: 'uppercase', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ display: 'block', width: 24, height: 2, background: 'var(--orange)' }} />Over Guido
          </div>
          <h2 style={{ ...D, fontSize: 'clamp(36px, 4.5vw, 60px)', letterSpacing: 1, lineHeight: 1, marginBottom: 24, fontWeight: 700 }}>VAN GOUDEN PLAK TOT SPORTINSTRUCTEUR</h2>
          ...
          <div className="milestones-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {[
              ...
              ['INSTRUCTOR', 'Instructeur Fysieke Training & Sport'],
            ].map(([year, text]) => (
      ...
      {/* DIENSTEN */}
      <section id="diensten" className="section-padding" style={{ padding: '100px 60px', background: 'var(--dark)' }}>
        <div style={{ ...B, fontSize: 10, letterSpacing: 4, color: 'var(--orange)', textTransform: 'uppercase', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ display: 'block', width: 24, height: 2, background: 'var(--orange)' }} />Wat ik aanbied
        </div>
        <h2 style={{ ...D, fontSize: 'clamp(42px, 5vw, 68px)', letterSpacing: 1, lineHeight: 1, marginBottom: 60, fontWeight: 700 }}>DIENSTEN</h2>
        <div className="diensten-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
          {[
            ...
            { icon: '🥊', title: 'GROEPSLESSEN & INHUUR', desc: 'Certified instructeur voor spinning, boxing en small group training. Beschikbaar voor sportscholen, clubs en events.', tag: 'Spinning · Boxing · Small Group' },
          ].map((d) => (
      ...
      {/* HOE HET WERKT */}
      <section id="werkwijze" className="section-padding" style={{ padding: '100px 60px', background: 'var(--dark2)', textAlign: 'center' }}>
        ...
        <div className="process-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
          {[
      ...
      {/* CERTIFICATEN */}
      <section id="kwalificaties" className="section-padding" style={{ padding: '100px 60px', background: 'var(--dark)' }}>
        <div style={{ ...B, fontSize: 10, letterSpacing: 4, color: 'var(--orange)', textTransform: 'uppercase', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ display: 'block', width: 24, height: 2, background: 'var(--orange)' }} />Achtergrond & Kwalificaties
        </div>
      ...
      {/* FOOTER */}
      <footer className="site-footer" style={{ background: 'var(--dark)', borderTop: '1px solid rgba(255,77,0,0.1)', padding: '40px 60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
