...
export default async function ClientDetail({ params }) {
  ...
  const { data: testResults } = await supabaseAdmin
    .from('test_results').select('*').eq('client_id', id)
    .order('test_date', { ascending: false })

  const { data: checkIns } = await supabaseAdmin
    .from('daily_checkins').select('*').eq('client_id', id)
    .order('checkin_date', { ascending: false }).limit(10)
      ...
      <main style={{ padding: '40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 2, marginBottom: 24 }}>
          ...
          <div style={{ background: 'var(--dark2)', padding: 28 }}>
            ...
            {client.notes && <div style={{ ...B, fontSize: 13, color: '#aaa', fontStyle: 'italic', lineHeight: 1.6 }}>{client.notes}</div>}
          </div>

          {/* Huidig plan */}
          <div style={{ background: 'var(--dark2)', padding: 28 }}>
            <div style={{ ...B, fontSize: 10, letterSpacing: 4, color: 'var(--orange)', textTransform: 'uppercase', marginBottom: 20 }}>Huidig plan</div>
            {macroPlan ? (
              <>
                <div style={{ ...D, fontSize: 20, fontWeight: 700, letterSpacing: 1, marginBottom: 6 }}>{macroPlan.name}</div>
                <div style={{ ...B, fontSize: 13, color: 'var(--muted)', marginBottom: 16 }}>{macroPlan.goal}</div>
                <div style={{ display: 'flex', gap: 20, marginBottom: 16 }}>
                  <div>
                    <div style={{ ...B, fontSize: 10, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 3 }}>Start</div>
                    <div style={{ ...B, fontSize: 13 }}>{macroPlan.start_date ? new Date(macroPlan.start_date).toLocaleDateString('nl-NL') : '—'}</div>
                  </div>
                  <div>
                    <div style={{ ...B, fontSize: 10, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 3 }}>Einde</div>
