  ...
  return (
    <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#1a1a1a', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      {items.map(item => {
        ...
        return (
          <a key={item.id} href={item.href} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '10px 8px', color: isActive ? '#FF4D00' : '#666', textDecoration: 'none', ...B }}>
            {item.svg}
  ...
  return (
    <header style={{ background: '#1a1a1a', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '13px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <svg width="18" height="16" viewBox="0 0 36 34"><polygon points="18,2 13,28 23,28" fill="#FF4D00"/><polygon points="5,7 0,28 14,28" fill="#FF4D00" opacity="0.5"/><polygon points="31,7 23,28 36,28" fill="#FF4D00" opacity="0.5"/></svg>
...
export default function ClientDashboard() {
  ...
  if (loading) return (
    <div style={{ background: '#141414', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', ...D, fontSize: 20, color: '#FF4D00', letterSpacing: '3px' }}>
      LADEN...
    </div>
  )
      ...
      <main style={{ padding: '16px 16px 0' }}>
        ...
        <div style={{ marginBottom: 20, paddingTop: 4 }}>
          ...
          <div style={{ ...D, fontSize: 28, fontWeight: 700, letterSpacing: '0.5px', color: '#f0ede8' }}>
            Hey, <span style={{ color: '#FF4D00' }}>{data?.naam}</span>
          </div>
        </div>

        {/* Check-in banner */}
        {!data?.todayCI && (
          <a href="/dashboard/client/checkin" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,77,0,0.08)', border: '1px solid rgba(255,77,0,0.25)', borderRadius: 14, padding: '14px 16px', marginBottom: 16, textDecoration: 'none' }}>
            <div>
              <div style={{ ...D, fontSize: 14, fontWeight: 700, color: '#FF4D00', letterSpacing: '0.5px' }}>Dagelijkse check-in</div>
              <div style={{ ...B, fontSize: 12, color: '#666' }}>Nog niet ingevuld vandaag</div>
            </div>
            <div style={{ background: '#FF4D00', borderRadius: 8, padding: '8px 14px', ...B, fontSize: 11, fontWeight: 700, color: '#000', letterSpacing: '1px', textTransform: 'uppercase', flexShrink: 0, fontFamily: "'Barlow Condensed', sans-serif" }}>Invullen</div>
          </a>
        )}
        ...
        {data?.todaySess ? (
          <div style={{ background: '#1e1e1e', borderRadius: 16, border: '1px solid rgba(255,77,0,0.25)', padding: 18, marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14, gap: 12 }}>
              ...
              <a href={`/dashboard/client/session/${data.todaySess.id}`} style={{ background: '#FF4D00', borderRadius: 10, padding: '10px 18px', ...B, fontSize: 12, fontWeight: 700, color: '#000', letterSpacing: '1px', textTransform: 'uppercase', flexShrink: 0, textDecoration: 'none', fontFamily: "'Barlow Condensed', sans-serif" }}>Start →</a>
            </div>
            {data.todaySess.session_exercises?.slice(0,4).map((ex, i) => (
              <div key={ex.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <span style={{ ...D, fontSize: 12, color: '#FF4D00', fontWeight: 700, minWidth: 18 }}>{i+1}</span>
          ...
          <div style={{ background: '#1e1e1e', borderRadius: 16, border: '1px solid rgba(255,255,255,0.06)', padding: '24px 18px', textAlign: 'center', marginBottom: 16 }}>
            ...
            <a href="/dashboard/client/week" style={{ ...B, fontSize: 11, color: '#FF4D00', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', textDecoration: 'none', fontFamily: "'Barlow Condensed', sans-serif" }}>Bekijk weekschema →</a>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
          {DAYS.map((day, i) => {
            ...
            return (
              <a key={day} href="/dashboard/client/week" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '9px 9px', borderRadius: 10, background: isToday ? '#FF4D00' : '#1e1e1e', border: `1px solid ${isToday ? '#FF4D00' : session ? 'rgba(255,77,0,0.3)' : 'rgba(255,255,255,0.06)'}`, minWidth: 40, flexShrink: 0, textDecoration: 'none' }}>
                ...
                <div style={{ width: 5, height: 5, borderRadius: '50%', background: isToday ? '#000' : session ? '#FF4D00' : '#2a2a2a' }} />
              </a>
            )
          })}
        </div>
        ...
        {data?.feedback?.length > 0 && (<>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            {data.feedback.map(f => (
              <div key={f.id} style={{ background: '#1e1e1e', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', borderLeft: '3px solid #FF4D00', padding: '14px 16px' }}>
        ...
        {data?.recentCI?.length === 0 ? (
          <div style={{ background: '#1e1e1e', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', padding: '24px 18px', textAlign: 'center' }}>
            ...
            <a href="/dashboard/client/checkin" style={{ ...B, fontSize: 11, color: '#FF4D00', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', textDecoration: 'none', fontFamily: "'Barlow Condensed', sans-serif" }}>Eerste check-in →</a>
          </div>
        ) : (
            ...
            {data.recentCI.map(c => (
              <div key={c.id} style={{ background: '#1e1e1e', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: '11px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  {c.morning_weight && <span style={{ ...D, fontSize: 13, fontWeight: 700, color: '#FF4D00' }}>{c.morning_weight}kg</span>}
                </div>
              </div>
            ))}
...
function SectionLabel({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: "'Barlow Condensed', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '3px', color: '#FF4D00', textTransform: 'uppercase', marginBottom: 10 }}>
      <span style={{ display: 'block', width: 14, height: 2, background: '#FF4D00', borderRadius: 1, flexShrink: 0 }}/>
      {children}
    </div>
  )
}
...
function TypeBadge({ type }) {
  return (
    <span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: 20, background: 'rgba(255,77,0,0.1)', color: '#FF4D00', border: '1px solid rgba(255,77,0,0.25)', fontFamily: "'Barlow Condensed', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase' }}>
      {type}
    </span>
  )
}
